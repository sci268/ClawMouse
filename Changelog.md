...
- 以 `ClawMouse` 作为新的开源项目目录与品牌入口
- 新增 `ClawMouse.py`，作为兼容 GUI 与 CLI 的推荐启动入口
- MCP 能力整理为更适合 AI 调用的高层接口，包括 `trae_status` 与 `trae_delegate`
- 增加桥接提示词、状态检查、任务委托与示例工作流文档
- 新增 `CONTRIBUTING.md`、`SECURITY.md`、`CODE_OF_CONDUCT.md`、`docs/ARCHITECTURE.md`
- 更新 README、示例配置、忽略规则与打包入口，便于直接公开发布
- 主界面完成 ClawMouse 品牌化改版，补充手机编程链路说明与更清晰的视觉层级
- 新增桌面内置 MCP 面板，支持默认自动启动、帮助说明、启动/停止按钮与本地 HTTP 状态显示
- Windows 单文件 exe 已完成本地验证，这一版可以视为一个成功且稳定的里程碑版本

...
如果你想把手机也接进这条链路，ClawMouse 现在已经可以把：
...
- 你在手机上把需求发到微信
- 微信消息进入 Lobster 工作流
- Lobster 通过 ClawMouse 的窗口发送 / bridge 能力把任务稳定投递给 Trae 对话框
- Trae 在桌面上继续执行、写代码、改文件、跑检查
- ClawMouse 再把截图、窗口内容或执行结果回传给微信

这意味着你不只是“远程发一句话给 AI”，而是在手机上搭起了一条从消息入口、到桌面执行、再到结果回流的完整闭环。

## 手机编程链路

你可以把它理解成一个把手机、聊天软件、本地代理和桌面 IDE 串起来的编程通道：

```text
手机 -> 微信 -> Lobster -> ClawMouse MCP -> Trae -> ClawMouse 截图 / 窗口结果 -> 微信
```

这条链路的价值在于：

- 手机侧只负责发起需求，不需要直接操作桌面
- Lobster 负责理解、转发、编排和接续任务
- ClawMouse 负责把消息稳定送进目标窗口，并把桌面状态真实取回
- Trae 负责实际执行编码、修改和验证
- 微信负责把结果重新送回你的手机，形成低扰动、低门槛、可闭环的移动编程体验
...
- 手机编程协作，例如通过微信把任务发给 Lobster，再由 ClawMouse 稳定送入 Trae，并把截图与执行结果回传到手机
...
- 移动编程闭环：把微信、Lobster、Trae 和桌面截图/结果回传串成一条完整链路
